...
Class VideoController extends AbstractController {
    ...
    public function listAction() {
        if (isset($this->inputData['targetid']) && $this->inputData['targetid']) {
            ...
            $sql = 'SELECT id, name, url, cover_url coverUrl, targetid FROM t_xunfei_video_sub WHERE targetid = ? AND id > ? ORDER BY id' . $limit;
            ...
            if ($videoCount) {
                ...
                $sql = 'SELECT id, name, url, cover_url coverUrl, targetid FROM t_xunfei_video_sub WHERE targetid = ? AND id > 0 ORDER BY id' . $limit;
                $videoList = $this->db->getAll($sql, $this->inputData['targetid']);
            }
