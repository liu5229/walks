    ...
    foreach ($treeList->data->cols as $treeInfo) {
        ...
        while (true) {
            ...
            foreach ($subList->data as $subInfo) {
                $sql = 'INSERT INTO t_xunfei_video_sub (id, name, url, song, size, cover_url, targetid) SELECT ?, ?, ?, ?, ?, ?, ? FROM DUAL WHERE NOT EXISTS (SELECT sub_id FROM t_xunfei_video_sub WHERE id = ?)';
                $db->exec($sql, $subInfo->id, $subInfo->nm, $subInfo->url, isset($subInfo->song) ? json_encode($subInfo->song) : '', ROUND($subInfo->size / 1024 / 1024, 2), $subInfo->pvurl, $treeInfo->targetid, $subInfo->id);
